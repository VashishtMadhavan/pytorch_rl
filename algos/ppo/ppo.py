...
class PPO(A2C):
    ...
    def loss(self, sample_dict, idx):
        ...
        old_values = sample_dict['old_values']; returns = sample_dict['returns']
        # clipped pg loss
        ratio = torch.exp(log_ratios)
        pg_loss1 = -advantages.detach() * ratio
        pg_loss2 = -advantages.detach() * torch.clamp(ratio, min=1.0 - self.clip_frac, max=1.0 + self.clip_frac)
